...
func handleWebhook(w http.ResponseWriter, r *http.Request) {
    ...
    if event == "pull_request" {
        ...
        if len(matches) > 1 {
            ...
            link := map[string]any{"type": "pull_request", "url": prURL, "repo": repoFullName}
        ...
        if action == "closed" {
            ...
            if merged {
                ...
                if len(matches) > 1 {
                    ...
                    decision := map[string]any{"id": "pr-merged", "actor": actor, "message": "PR merged", "meta": map[string]any{"pr_url": prURL(body)}}
                    client := &http.Client{Timeout: 5 * time.Second}
