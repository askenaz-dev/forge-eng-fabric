...
func newDeploymentEvent(d *Deployment, eventType string, data map[string]any) CloudEvent {
	return CloudEvent{
		SpecVersion:        "1.0",
		ID:                 newID(),
		Source:             "forge://service/deploy-orchestrator",
		Type:               eventType,
		Subject:            "deployment/" + d.ID,
		Time:               time.Now().UTC(),
		DataContentType:    "application/json",
		ForgeTenantID:      d.TenantID,
		ForgeWorkspaceID:   d.WorkspaceID,
		ForgeActor:         d.Actor,
		ForgeCorrelationID: d.CorrelationID,
		Data:               data,
	}
}
