...
Forge `asset-observability` service Helm chart.

## Purpose

Deploys the Forge `asset-observability` service. This chart is generated from the `service-http` flavor under `infra/helm/_flavors/service-http/`.
...
```sh
helm install asset-observability infra/helm/asset-observability \
  --values infra/helm/asset-observability/values.yaml \
  --values infra/helm/asset-observability/values-local.yaml
```
...
cosign verify-blob \
  ...
  --signature asset-observability-0.1.0.tgz.sig \
  asset-observability-0.1.0.tgz
```
