...
Forge `healing-engine` service Helm chart.

## Purpose

Deploys the Forge `healing-engine` service. This chart is generated from the `service-worker` flavor under `infra/helm/_flavors/service-worker/`.
...
```sh
helm install healing-engine infra/helm/healing-engine \
  --values infra/helm/healing-engine/values.yaml \
  --values infra/helm/healing-engine/values-local.yaml
```
...
cosign verify-blob \
  ...
  --signature healing-engine-0.1.0.tgz.sig \
  healing-engine-0.1.0.tgz
```
