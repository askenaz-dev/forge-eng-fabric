...
Forge `rag-query` service Helm chart.

## Purpose

Deploys the Forge `rag-query` service. This chart is generated from the `service-worker` flavor under `infra/helm/_flavors/service-worker/`.
...
```sh
helm install rag-query infra/helm/rag-query \
  --values infra/helm/rag-query/values.yaml \
  --values infra/helm/rag-query/values-local.yaml
```
...
cosign verify-blob \
  ...
  --signature rag-query-0.1.0.tgz.sig \
  rag-query-0.1.0.tgz
```
