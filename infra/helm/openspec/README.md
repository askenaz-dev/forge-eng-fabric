...
Forge `openspec` service Helm chart.

## Purpose

Deploys the Forge `openspec` service. This chart is generated from the `service-http` flavor under `infra/helm/_flavors/service-http/`.
...
```sh
helm install openspec infra/helm/openspec \
  --values infra/helm/openspec/values.yaml \
  --values infra/helm/openspec/values-local.yaml
```
...
cosign verify-blob \
  ...
  --signature openspec-0.1.0.tgz.sig \
  openspec-0.1.0.tgz
```
