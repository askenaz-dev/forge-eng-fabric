...
Forge `app-onboarding` service Helm chart.

## Purpose

Deploys the Forge `app-onboarding` service. This chart is generated from the `service-http` flavor under `infra/helm/_flavors/service-http/`.
...
```sh
helm install app-onboarding infra/helm/app-onboarding \
  --values infra/helm/app-onboarding/values.yaml \
  --values infra/helm/app-onboarding/values-local.yaml
```
...
cosign verify-blob \
  ...
  --signature app-onboarding-0.1.0.tgz.sig \
  app-onboarding-0.1.0.tgz
```
