...
Forge `eval-harness-adv` service Helm chart.

## Purpose

Deploys the Forge `eval-harness-adv` service. This chart is generated from the `service-http` flavor under `infra/helm/_flavors/service-http/`.
...
```sh
helm install eval-harness-adv infra/helm/eval-harness-adv \
  --values infra/helm/eval-harness-adv/values.yaml \
  --values infra/helm/eval-harness-adv/values-local.yaml
```
...
cosign verify-blob \
  ...
  --signature eval-harness-adv-0.1.0.tgz.sig \
  eval-harness-adv-0.1.0.tgz
```
