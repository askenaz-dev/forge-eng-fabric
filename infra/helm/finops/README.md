...
Forge `finops` service Helm chart.

## Purpose

Deploys the Forge `finops` service. This chart is generated from the `service-http` flavor under `infra/helm/_flavors/service-http/`.
...
```sh
helm install finops infra/helm/finops \
  --values infra/helm/finops/values.yaml \
  --values infra/helm/finops/values-local.yaml
```
...
cosign verify-blob \
  ...
  --signature finops-0.1.0.tgz.sig \
  finops-0.1.0.tgz
```
