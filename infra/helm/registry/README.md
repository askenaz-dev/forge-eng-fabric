...
Forge `registry` service Helm chart.

## Purpose

Deploys the Forge `registry` service. This chart is generated from the `service-http` flavor under `infra/helm/_flavors/service-http/`.
...
```sh
helm install registry infra/helm/registry \
  --values infra/helm/registry/values.yaml \
  --values infra/helm/registry/values-local.yaml
```
...
cosign verify-blob \
  ...
  --signature registry-0.1.0.tgz.sig \
  registry-0.1.0.tgz
```
