...
Forge `incident-detection` service Helm chart.

## Purpose

Deploys the Forge `incident-detection` service. This chart is generated from the `service-worker` flavor under `infra/helm/_flavors/service-worker/`.
...
```sh
helm install incident-detection infra/helm/incident-detection \
  --values infra/helm/incident-detection/values.yaml \
  --values infra/helm/incident-detection/values-local.yaml
```
...
cosign verify-blob \
  ...
  --signature incident-detection-0.1.0.tgz.sig \
  incident-detection-0.1.0.tgz
```
