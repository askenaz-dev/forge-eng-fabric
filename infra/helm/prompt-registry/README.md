...
Forge `prompt-registry` service Helm chart.

## Purpose

Deploys the Forge `prompt-registry` service. This chart is generated from the `service-http` flavor under `infra/helm/_flavors/service-http/`.
...
```sh
helm install prompt-registry infra/helm/prompt-registry \
  --values infra/helm/prompt-registry/values.yaml \
  --values infra/helm/prompt-registry/values-local.yaml
```
...
cosign verify-blob \
  ...
  --signature prompt-registry-0.1.0.tgz.sig \
  prompt-registry-0.1.0.tgz
```
