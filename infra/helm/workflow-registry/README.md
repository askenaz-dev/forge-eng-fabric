...
Forge `workflow-registry` service Helm chart.

## Purpose

Deploys the Forge `workflow-registry` service. This chart is generated from the `service-http` flavor under `infra/helm/_flavors/service-http/`.
...
helm install workflow-registry infra/helm/workflow-registry \
  --values infra/helm/workflow-registry/values.yaml \
  --values infra/helm/workflow-registry/values-local.yaml
```
...
cosign verify-blob \
  ...
  --signature workflow-registry-0.1.0.tgz.sig \
  workflow-registry-0.1.0.tgz
```
