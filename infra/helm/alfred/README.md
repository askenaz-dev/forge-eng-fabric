...
Forge `alfred` service Helm chart.

## Purpose

Deploys the Forge `alfred` service. This chart is generated from the `service-http` flavor under `infra/helm/_flavors/service-http/`.
...
```sh
helm install alfred infra/helm/alfred \
  --values infra/helm/alfred/values.yaml \
  --values infra/helm/alfred/values-local.yaml
```
...
cosign verify-blob \
  ...
  --signature alfred-0.1.0.tgz.sig \
  alfred-0.1.0.tgz
```
