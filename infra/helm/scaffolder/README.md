...
Forge `scaffolder` service Helm chart.

## Purpose

Deploys the Forge `scaffolder` service. This chart is generated from the `service-http` flavor under `infra/helm/_flavors/service-http/`.
...
```sh
helm install scaffolder infra/helm/scaffolder \
  --values infra/helm/scaffolder/values.yaml \
  --values infra/helm/scaffolder/values-local.yaml
```
...
cosign verify-blob \
  ...
  --signature scaffolder-0.1.0.tgz.sig \
  scaffolder-0.1.0.tgz
```
