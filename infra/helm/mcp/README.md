...
Forge `mcp` service Helm chart.

## Purpose

Deploys the Forge `mcp` service. This chart is generated from the `service-http` flavor under `infra/helm/_flavors/service-http/`.
...
```sh
helm install mcp infra/helm/mcp \
  --values infra/helm/mcp/values.yaml \
  --values infra/helm/mcp/values-local.yaml
```
