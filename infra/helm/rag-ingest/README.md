...
Forge `rag-ingest` service Helm chart.

## Purpose

Deploys the Forge `rag-ingest` service. This chart is generated from the `service-worker` flavor under `infra/helm/_flavors/service-worker/`.
...
```sh
helm install rag-ingest infra/helm/rag-ingest \
  --values infra/helm/rag-ingest/values.yaml \
  --values infra/helm/rag-ingest/values-local.yaml
```
...
cosign verify-blob \
  ...
  --signature rag-ingest-0.1.0.tgz.sig \
  rag-ingest-0.1.0.tgz
```
