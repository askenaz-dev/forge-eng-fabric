...
Forge `incidents-kb` service Helm chart.

## Purpose

Deploys the Forge `incidents-kb` service. This chart is generated from the `service-http` flavor under `infra/helm/_flavors/service-http/`.
...
```sh
helm install incidents-kb infra/helm/incidents-kb \
  --values infra/helm/incidents-kb/values.yaml \
  --values infra/helm/incidents-kb/values-local.yaml
```
...
cosign verify-blob \
  ...
  --signature incidents-kb-0.1.0.tgz.sig \
  incidents-kb-0.1.0.tgz
```
