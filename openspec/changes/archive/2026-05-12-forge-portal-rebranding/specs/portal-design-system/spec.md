...
The Portal SHALL self-host the brand typefaces from `portal/public/fonts/`: **Instrument Serif** (regular + italic) for display, **Geist** (regular + medium + 500) for UI, and **JetBrains Mono** (regular + italic) for monospace. The CSS variables `--f-display`, `--f-sans`, `--f-mono` SHALL map to these families with system fallbacks. `<link>` tags SHALL include `preload` for the four most-used weights. No `fonts.googleapis.com` or `fonts.gstatic.com` request SHALL be issued by the Portal at runtime.
...
- **AND** the network panel records the font being loaded from `/fonts/instrument-serif-regular.woff2`, not from any Google domain
...
- **THEN** CI fails on the `no-portal-mocks` lint rule and the reviewer rejects the change
