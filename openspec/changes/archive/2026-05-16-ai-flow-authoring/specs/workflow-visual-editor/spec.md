...
**Reason:** Replaced by gateway-resolved invocation at execution time. The previous design pinned the gateway endpoint into the saved AST to avoid runtime resolution, which has caused breakage when endpoints rotate. The runtime now always resolves through `mcp-gateway` and `a2a-gateway` at dispatch, so the saved AST holds only the canonical asset reference (`id@version`).
**Migration:** Existing saved workflows with `node.active_surface.endpoint` populated continue to work — the field is ignored at dispatch. New saves SHALL omit the endpoint. A one-time migration during this change SHALL strip `active_surface.endpoint` from existing persisted versions and re-persist them as patch-level updates with reason `cleanup_active_surface_endpoint`.
