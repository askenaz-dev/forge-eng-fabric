# portal-i18n Specification

## Purpose

Bilingual (Spanish-default / English) internationalisation layer for the
Forge Portal: typed dictionary, `useLang()` hook, variable substitution,
server-rendered initial language, server-persisted preference, audit events,
and a glossary of canonical platform-term translations. Created by archiving
change `forge-portal-rebranding`.

## Requirements

### Requirement: Bilingual content layer with Spanish default

The Portal SHALL ship with two supported locales — `es` (default) and `en` — backed by a static, type-safe dictionary at `portal/src/i18n/dictionary.ts`. The dictionary SHALL define every translatable string keyed by short identifiers (e.g. `nav_dashboard`, `h_invite`, `apr_approve`, `toast_theme`, `kpi_p95`, `runs_filter_running`). Every key SHALL exist for both locales — missing keys SHALL fall back to `es` and SHALL be reported as a build-time error.
...
When `setLang(next)` is called, the Portal SHALL `POST /api/i18n/preference { "lang": next }` which writes to `users.preferences.locale` via the user-preferences endpoint of the control-plane. Toasts SHALL confirm the switch (`toast_lang` / `toast_lang_en`) and a `portal.lang.changed.v1` audit event SHALL be emitted with the principal, previous and next locales.

#### Scenario: Lang switch is audited

- **WHEN** a user switches from `es` to `en`
- **THEN** the audit log contains an entry `{ "type": "portal.lang.changed.v1", "principal": "...", "data": { "to": "en", "from": "es" }, "correlation_id": "..." }`

### Requirement: Pluralisation and number/date formatting

The `t()` API SHALL accept `count` to select a plural form. Number and date formatting SHALL use the Intl APIs with the active locale (`new Intl.NumberFormat(lang)`, `new Intl.DateTimeFormat(lang, { dateStyle: 'medium', timeStyle: 'short' })`). Durations expressed in seconds SHALL be formatted as `mm:ss` regardless of locale via `formatDuration(seconds)`. Relative times SHALL use `Intl.RelativeTimeFormat` with `numeric: "auto"` for human-friendly output.
...
- **THEN** the displayed value uses `Intl.DateTimeFormat('es', { dateStyle: 'medium', timeStyle: 'short' })`
- **AND** under `en` the same timestamp uses `Intl.DateTimeFormat('en', …)`
