# alfred-agent-mode Specification

## Purpose

Long-running, plan-driven Alfred session that orchestrates intent-to-deploy
end-to-end with policy-respecting autonomy, HITL pauses at required gates,
a session SSE stream consumed by the portal dock, and resumability across
pauses and disconnects. Created by archiving change
...
### Requirement: Agent-mode session is a first-class, long-running object (updated)
