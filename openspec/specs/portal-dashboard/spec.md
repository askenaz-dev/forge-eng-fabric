# portal-dashboard Specification

## Purpose

The canonical Tablero landing page for the Forge Portal — time-of-day
greeting, serif italic-accent headline, KPI grid, runs table, approval queue,
audit-event timeline, services-mesh visualization and run detail sheet. Every
panel binds to live platform APIs and re-renders via SSE. Created by
archiving change `forge-portal-rebranding`.
...
Below the headline the dashboard SHALL render two CTA buttons: secondary "Invitar equipo" / "Invite team" and primary ember "Lanzar workflow" / "Launch workflow". Buttons SHALL use the design-system `Button` primitive.

#### Scenario: Lanzar workflow opens the picker

- **WHEN** the user clicks "Lanzar workflow"
- **THEN** the page routes to `/workflows` where a workflow picker is available

### Requirement: KPI grid

The dashboard SHALL render a four-column KPI grid populated from `/api/observability/kpis?window=24h`. The four KPIs SHALL be:
...
- **THEN** `/api/observability/kpis?window=24h` is called, the response is rendered into the four KPI cards, and a `x-correlation-id` header is propagated for traceability
...
The dashboard SHALL render the "Runs recientes" table fed by `/api/sdlc/runs?limit=50&order=desc` with the schema described below per row:
...
- **THEN** the right-side sheet opens with header `Run · Deploy v1.42.0 → prod` and body populated from `/api/sdlc/runs/wf_8a13c1` containing the step list, durations and tones, and the URL bar updates to `/?run=wf_8a13c1`

### Requirement: Cola de aprobación stack

The dashboard SHALL render the approval queue card sourced from `/api/approvals?status=pending&approver=<me>&limit=10` showing for each item: the requesting agent's two-letter tag, title (localised), meta (run id, branch, scope), severity badge (`high` if applicable), diff summary (`+N / −M` and one-line summary), inline action buttons `Aprobar` / `Revisar` / `Rechazar`, and an expiry timer counting down to `expires_at`. Approving / rejecting SHALL call `POST /api/approvals/{id}/decisions` and remove the item from the list on success.

#### Scenario: Approving removes the item and emits an audit event

- **WHEN** the user clicks `Aprobar` on `apr_2401`
- **THEN** a `POST /api/approvals/apr_2401/decisions {"decision":"approve","actor":"<me>"}` is sent, the row animates out, the sidebar pill count for `Aprobaciones` decrements by one, and `approvals.granted.v1` appears in the audit stream
...
The dashboard SHALL render a signed-events timeline sourced from `/api/audit/events?limit=20&scope=workspace` with iconography keyed to event type:
...
Each row SHALL render the event text from the i18n key `ev_*` with variables bolded inline (e.g. `Agent <code>review-bot</code> started a run on <code>acme/payments-svc</code>`), and a relative "when" stamp (e.g. `hace 2 min` / `2 min ago`).
...
The eyebrow under each card SHALL show "en vivo · refresca cada Ns" reflecting either the SSE pulse or a fallback polling interval. No fixed-interval poller SHALL be active when SSE is connected.

#### Scenario: New run via SSE appears at the top

- **WHEN** SSE emits `agent.run.started.v1 { run: { id: "wf_8a13d0", agent: "doc-weaver", ... } }`
- **THEN** within one frame the new row is prepended to the runs table and the `Runs en curso` KPI re-fetches
